...
description: |
  Set up Higgsfield AI generation capabilities on a fresh server or after auth
  breakage. Covers CLI install, credential transfer from Mac, workspace config,
  and first-generation smoke test. Use when: "set up Higgsfield", "fix Higgsfield
  auth", "Higgsfield not working", "reinstall Higgsfield", "new server Higgsfield".
